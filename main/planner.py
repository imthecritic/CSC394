...
class Planner:
    ...
    def plan(self, courses, taken, start, rate,credits):
        ...
        term = start
        options = self.getSuccessors(allclasses,courses, courses_taken, rate, term,0)
        cls_cntr = 0
        ...
        while(len(options != 0)):
            ...
            if self.isGoal(current, credits):
                ...
                if cls_cntr == rate:#number of classes for term has been acheived
                    cls_cntr = 0
                    term = (term + 1) % 4 #set new term
                
            options += self.getSuccessors(allclasses, courses, courses_taken, rate,term,current[1])
    ...
    def getSuccessors(self, allcourse, courses,courses_taken, rate, term,current_total):
        options = []
        for course in courses:
            if self.validPrereq(course, courses_taken) and  self.offered(course,term): #course is valid insert to options
                #check if 
...
class schedule_state:
    ...
    #current term
    #current estimate
    def __init__(self, tkn, avail, trm):
        self.taken      = tkn
